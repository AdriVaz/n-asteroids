...
int main(int argc, char const *argv[])
{
	...
	for(unsigned int i = 0; i < argvNum.size() && validParams; i++)
	{
		if(argvNum[i] <= 0)
			validParams = false;
	}
	...
	for(int iterations = 0; iterations < argvNum[1];iterations++)
	{
		...
		//vector <bool> asteroidProcessed(asteroids.size());

		//Calculo de los rebotes entre asteroides
		for(unsigned int i = 0; i < asteroids.size()/* && !asteroidProcessed[i]*/; ++i)
		{
